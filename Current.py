...
# 구간별 평균 Current 계산
current_grouped = df_filtered.groupby('interval')['Current'].mean()

# 꺾은선 그래프 시각화 (Current only)
plt.figure()
plt.plot(current_grouped.index, current_grouped.values, marker='s')
plt.xlabel('Time Interval (s)')
plt.ylabel('Current')
plt.title('Current per 5-Second Interval (1s to 100s)')
